...
void	ft_put_pixel(t_mlx mlx_data, t_point point)
{
	...
	if (point.x < 0 || point.x >= mlx_data.windows_size_x
		|| point.y < 0 || point.y >= mlx_data.windows_size_y)
		return ;
	...
	while (y < mlx_data.map_size_y)
	{
		...
		while (x < mlx_data.map_size_x)
		{
			if (x < mlx_data.map_size_x - 1)
				ft_put_line(mlx_data, tab_point[y][x].proj,
					tab_point[y][x + 1].proj);
			if (y < mlx_data.map_size_y - 1)
				ft_put_line(mlx_data, tab_point[y][x].proj,
					tab_point[y + 1][x].proj);
			x++;
		}
		y++;
	}
...
void	ft_create_image(t_mlx *mlx_data)
{
	...
	mlx_data->img.img_ptr = mlx_new_image(mlx_data->addr_init,
			mlx_data->windows_size_x, mlx_data->windows_size_y);
	mlx_data->img.img_data = mlx_get_data_addr(mlx_data->img.img_ptr,
			&mlx_data->img.bpp, &mlx_data->img.size_line,
			&mlx_data->img.endian);
	ft_draw(*mlx_data, mlx_data->tab_point);
	mlx_put_image_to_window(mlx_data->addr_init, mlx_data->addr_windows,
