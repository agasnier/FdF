...
static int	ft_new_windows(t_mlx *mlx_data)
{
	...
	mlx_get_screen_size(mlx_data->addr_init, &mlx_data->windows_size_x,
		&mlx_data->windows_size_y);
	mlx_data->addr_windows = mlx_new_window(mlx_data->addr_init,
			mlx_data->windows_size_x, mlx_data->windows_size_y,
			mlx_data->windows_title);
...
static void	ft_init_struct_mlx(t_mlx *mlx_data)
{
